...
class ChartPreviewApp:
    def __init__(self, root):
        ...
        file_frame = tk.Frame(root, bg='#1a1a24')
        file_frame.pack(pady=5, padx=20, fill='x')
        
        tk.Label(file_frame, text="OSU File:", bg='#1a1a24', fg='#aaaaaa').pack(side='left')
        
        self.file_entry = tk.Entry(file_frame, textvariable=self.file_path, width=25, bg='#252533', fg='#ffffff', insertbackground='#ffffff')
        self.file_entry.pack(side='left', padx=5)
        
        tk.Button(file_frame, text="Browse", command=self.browse_file, bg='#3a3a5c', fg='#ffffff').pack(side='left')
        
        settings_frame = tk.LabelFrame(root, text="Settings", bg='#1a1a24', fg='#ffffff', font=("Arial", 12))
        settings_frame.pack(pady=10, padx=20, fill='x')
        
        tk.Label(settings_frame, text="Spacing (px):", bg='#1a1a24', fg='#aaaaaa').grid(row=0, column=0, sticky='w', pady=5)
        ...
        spacing_px_entry.grid(row=0, column=1, sticky='w', pady=5)
        
        tk.Label(settings_frame, text="Reference (ms):", bg='#1a1a24', fg='#aaaaaa').grid(row=1, column=0, sticky='w', pady=5)
        ...
        spacing_ms_entry.grid(row=1, column=1, sticky='w', pady=5)
        
        tk.Label(settings_frame, text="Column Gap (px):", bg='#1a1a24', fg='#aaaaaa').grid(row=2, column=0, sticky='w', pady=5)
    ...
    def generate(self):
        ...
        num_keys = cs if cs > 0 else 4
        output_path = f"chart_preview_{num_keys}k.png"
        
        success = generate_chart_preview(objects, num_keys, output_path, 1, spacing_px, spacing_ms, col_gap, canvas_height)
